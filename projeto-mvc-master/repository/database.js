...
class Database {
    ...
    async insertSkin( categoria, nome, descr, genero, raridade, valor, foto1, foto2 ) {
        const sql = `
            INSERT INTO skins (categoria_skin, nome_skin, descr_skin, genero_skin, raridade_skin,
            valor_skin, foto1_skin, foto2_skin)
    ...
    async insertPromo(nome, start, end,  desc, ativa, desconto) {
        const sql = `
            INSERT INTO promocoes (nome_promocao, dt_start_promocao, dt_end_promocao, descr_promocao, ativa_promocao, descontos_id_desconto)
            VALUES ('${nome}', '${start}', '${end}', '${desc}', '${ativa}', '${desconto}');
        `;
        const bd = await this.#connection.execute(sql);
        return bd[0];
    }
    ...
    async updatePromocao( id, nome, start, end, desc, ativa, desconto ) {
     ...
     async selecionarDescontos() {
        const descontoData = await this.#connection.query("SELECT * from promocoes INNER JOIN descontos ON promocoes.descontos_id_desconto = descontos.id_desconto;")
    ...
    async deleteDesconto(id) {
      ...
      async insertVenda(dia, hora) {
        const sql = `
            INSERT INTO vendas (dia_venda, hora_venda)
            VALUES ('${dia}', '${hora}');
        `;
        const bd = await this.#connection.execute(sql);
        return bd[0];
